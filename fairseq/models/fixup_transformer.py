...
@register_model('fixup_transformer')
class FixupTransformerModel(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--Tfixup', action='store_true', default=False)
    ...
    def build_model(cls, args, task):
        ...
        encoder = FixupTransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = FixupTransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return FixupTransformerModel(encoder, decoder)


class FixupTransformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        self.layers.extend([
            FixupTransformerEncoderLayer(args)
        ...
        if self.normalize and not args.dont_use_layernorm:
...
class FixupTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, left_pad=False, final_norm=True):
        ...
        self.embed_tokens = embed_tokens
        self.embed_scale = 1.0 if args.no_scale_embedding else math.sqrt(embed_dim)  # todo: try with input_embed_dim
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            FixupTransformerDecoderLayer(args, no_encoder_attn)
...
class FixupTransformerEncoderLayer(nn.Module):
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        """
        residual = x
        x = self.maybe_layer_norm(0, x, before=True, noLN=self.noLN)
        x, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        x = self.maybe_layer_norm(0, x, after=True, noLN=self.noLN)

        residual = x
        x = self.maybe_layer_norm(1, x, before=True, noLN=self.noLN)
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=self.relu_dropout, training=self.training)
        x = self.fc2(x)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
        x = self.maybe_layer_norm(1, x, after=True, noLN=self.noLN)
        return x

    def maybe_layer_norm(self, i, x, before=False, after=False, noLN=False):
        assert before ^ after
        if after ^ self.normalize_before and not noLN:
...
@register_model_architecture('fixup_transformer', 'fixup_transformer')
def base_architecture(args):
    ...
    args.Tfixup = getattr(args, "Tfixup", False)
    args.dont_use_layernorm = getattr(args, "dont_use_layernorm", False)

@register_model_architecture('fixup_transformer', 'fixup_transformer_iwslt_de_en')
...
@register_model_architecture('fixup_transformer', 'fixup_transformer_wmt_en_de')
def transformer_wmt_en_de(args):
    args.encoder_layers = 20
    args.dont_use_layernorm = True
    args.Tfixup = True
    base_architecture(args)


@register_model_architecture('fixup_transformer', 'fixup_transformer_t2t_wmt_en_de')
...
@register_model_architecture('fixup_transformer', 'fixup_relative_transformer_wmt_en_de')
...
@register_model_architecture('fixup_transformer', 'fixup_relative_transformer_t2t_wmt_en_de')
...
@register_model_architecture('fixup_transformer', 'fixup_transformer_vaswani_wmt_en_de_big')
...
@register_model_architecture('fixup_transformer', 'fixup_transformer_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('fixup_transformer', 'fixup_transformer_wmt_en_de_big')
...
@register_model_architecture('fixup_transformer', 'fixup_transformer_wmt_en_de_big_t2t')
