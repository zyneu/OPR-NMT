...
@register_model('stochastic_lightconv')
class StochasticLightConvModel(FairseqModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        return StochasticLightConvModel(encoder, decoder)
...
class LightConvEncoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask, layer_num):
        ...
        preserve_attn = (torch.rand(1)[0].item() >= p_attn)
        preserve_ffn = (torch.rand(1)[0].item() >= p_fnn)
...
@register_model_architecture('stochastic_lightconv', 'stochastic_lightconv')
...
@register_model_architecture('stochastic_lightconv', 'stochastic_lightconv_wmt_en_de')
def lightconv_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('stochastic_lightconv', 'stochastic_lightconv_t2t_wmt_en_de')
