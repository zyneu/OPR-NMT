...
@register_model('multi_scale_transformer')
class MultiScaleTransformerModel(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        ### multi scale
        parser.add_argument('--scale_num', type=int, help='the number of the whole scale')
    ...
    def build_model(cls, args, task):
        ...
        encoder = MultiScaleTransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = MultiScaleTransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return MultiScaleTransformerModel(encoder, decoder)



class MultiScaleTransformerEncoder(FairseqEncoder):
    """
...
class MultiScaleTransformerDecoder(FairseqIncrementalDecoder):
    """
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.fc2_weight = nn.Parameter(torch.Tensor(self.embed_dim, args.encoder_ffn_embed_dim))
        self.fc2_bias = nn.Parameter(torch.Tensor(self.embed_dim))
        nn.init.xavier_uniform_(self.fc2_weight)
        nn.init.constant_(self.fc2_bias, 0.)
        self.enccoder_ffn_slices = args.encoder_ffn_slices
    ...
    def multi_scale_ffn(self, x):
        residual = x
        x = self.maybe_layer_norm(1, x, before=True)
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=self.relu_dropout, training=self.training)
        x = torch.stack([F.linear(x[:, :, s[0]:s[1]], self.fc2_weight[:, s[0]:s[1]], self.fc2_bias) for s in self.enccoder_ffn_slices])
        x = torch.mean(x, 0)
        # x = self.fc2(x)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
...
@register_model_architecture('multi_scale_transformer', 'multi_scale_transformer')
def base_architecture(args):
    ...
    args.encoder_ffn_slices = getattr(args, 'encoder_ffn_slices', None)


@register_model_architecture('multi_scale_transformer', 'multi_scale_transformer_iwslt_de_en')
def multi_scale_transformer_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    ...
    args.decoder_layers = getattr(args, 'decoder_layers', 6)
    args.encoder_ffn_slices = getattr(args, 'encoder_ffn_slices', [[0, 1024], [0, 512], [256, 768], [512, 1024], [0, 256], [256, 512], [512, 768],[768, 1024]])
    base_architecture(args)


@register_model_architecture('multi_scale_transformer', 'multi_scale_transformer_wmt_en_de')
def multi_scale_transformer_wmt_en_de(args):
...
@register_model_architecture('multi_scale_transformer', 'multi_scale_transformer_t2t_wmt_en_de')
def multi_scale_transformer_t2t_wmt_en_de(args):
...
@register_model_architecture('multi_scale_transformer', 'multi_scale_relative_transformer_wmt_en_de')
def multi_scale_relative_transformer_wmt_en_de(args):
    args.max_relative_length = 20
    args.encoder_layers = 6
    multi_scale_transformer_wmt_en_de(args)



@register_model_architecture('multi_scale_transformer', 'multi_scale_relative_transformer_t2t_wmt_en_de')
def multi_scale_relative_transformer_t2t_wmt_en_de(args):
    args.max_relative_length = 8
    args.encoder_layers = 6
    args.encoder_ffn_slices = getattr(args, 'encoder_ffn_slices',
                                      [[0, 2048], [0, 1024], [512, 1536], [1024, 2048], [0, 512], [512, 1024], [1024, 1536],
                                       [1536, 2048]])
    multi_scale_transformer_t2t_wmt_en_de(args)
