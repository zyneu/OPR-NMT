...
from fairseq.modules import (
    ...
    RelativeMultiheadAttention, OPRPositionalEmbedding,
)
...
class TransformerModel(FairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--opr-k-value', type=int, default=-1,
                            help='the k value for opr')
        # fmt: on
...
def PositionalEmbedding(num_embeddings, embedding_dim, padding_idx, left_pad, learned=False, opr_k_value=-1):
    if learned:
        m = LearnedPositionalEmbedding(num_embeddings + padding_idx + 1, embedding_dim, padding_idx, left_pad)
        nn.init.normal_(m.weight, mean=0, std=embedding_dim ** -0.5)
        nn.init.constant_(m.weight[padding_idx], 0)
    elif opr_k_value > 0:
        m = OPRPositionalEmbedding(embedding_dim, padding_idx, left_pad, num_embeddings + padding_idx + 1, opr_k_value)
    else:
...
def base_architecture(args):
    ...
    args.opr_k_value = getattr(args, 'opr_k_value', args.opr_k_value)
...
@register_model_architecture('transformer', 'opr_transformer_t2t_wmt_en_de')
def opr_transformer_t2t_wmt_en_de(args):
    args.encoder_normalize_before = True
    args.decoder_normalize_before = True
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.relu_dropout = getattr(args, 'relu_dropout', 0.1)
...
@register_model_architecture('transformer', 'opr_relative_transformer_wmt_en_de')
def opr_relative_transformer_wmt_en_de(args):
    args.encoder_normalize_before = True
    args.decoder_normalize_before = True
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.relu_dropout = getattr(args, 'relu_dropout', 0.1)
